...
class NotesController extends Controller
{
    ...
    public function getNotes(Request $request)
    {
        ...
        $displayOrder = config('notes-field.display_order', 'DESC');
        $notes = $model->notes()->orderBy('created_at', $displayOrder)->orderBy('id', $displayOrder)->get();

        return response()->json([
            'date_format' => config('notes-field.date_format', 'dd MMM yyyy HH:mm'),
            'trix_enabled' => config('notes-field.use_trix_input', false),
            'notes' => $notes,
        ], 200);
    }
